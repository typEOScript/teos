...
class Contract {
  constructor(name) {
    const configPath = path.join(process.cwd(), 'tos.config.js');
    if (!fs.existsSync(configPath)) {
      throw new Error('No config file tos.config.js');
    }
    const config = require(configPath);
    if (config.keyProvider.length === 0) {
      throw new Error('No private keys found in keyProvider of tos.config.js')
    }
